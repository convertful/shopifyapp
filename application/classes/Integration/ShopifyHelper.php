...
class Integration_ShopifyHelper {


// Helper method to determine if a shop domain is valid
	static function validateShopDomain( $shop ) {
...
// Helper method to determine if a request is valid
	static function validateHmac( $params, $secret ) {
		$hmac = $params['hmac'];
		unset( $params['hmac'] );
		ksort( $params );
...
// Helper method for exchanging credentials
	static function getAccessToken( $shop, $apiKey, $secret, $code ) {
		$query = array(
			'client_id'     => $apiKey,
...
// Helper method for making Shopify API requests
	static function performShopifyRequest( $shop, $token, $resource, $params = array(), $method = 'GET' ) {

		if ( $method == 'DELETE' ) {
			$url = "https://{$shop}/admin/{$resource}/{$params['id']}.json";
		} else {
		...
		if ( $method == 'POST' || $method == 'PUT' ) {
			...
			if ( ! is_null( $params ) && $method != 'DELETE' ) {
